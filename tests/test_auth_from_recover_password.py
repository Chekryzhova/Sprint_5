...
class TestAuthFromRecoverPassword:
    def test_auth_from_recover_password(self, driver):
        login_to_account = driver.find_element(*StellarburgersLocators.LOGIN_TO_ACCOUNT_BUTTON)
        login_to_account.click()
        recover_password_button = driver.find_element(*StellarburgersLocators.RECOVER_PASSWORD_BUTTON)
        recover_password_button.click()
        login_button_in_recovery_form = driver.find_element(*StellarburgersLocators.LOGIN_BUTTON_IN_REGISTRATION_OR_RECOVERY_FORM)
